...
/// An item in a menu created by a [FixDropdownButton].
///
...
class FixDropdownMenuItem<T> extends StatelessWidget {
  ...
  ///
  /// Eventually returned in a call to [FixDropdownButton.onChanged].
  final T value;
...
/// An inherited widget that causes any descendant [FixDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [FixDropdownButton] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [FixDropdownButton] widgets should
  /// be hidden.
...
class FixDropdownButton<T> extends StatefulWidget {
  ...
  FixDropdownButton({
    ...
    this.isDense = false,
        ...
        assert(value == null ||
            items
                    .where((FixDropdownMenuItem<T> item) => item.value == value)
                    .length ==
                1),
        super(key: key);
...
class _DropdownButtonState<T> extends State<FixDropdownButton<T>>
  ...
  @override
  void didUpdateWidget(FixDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    assert(widget.value == null ||
        widget.items
                .where(
                    (FixDropdownMenuItem<T> item) => item.value == widget.value)
                .length ==
            1);
    _selectedIndex = null;
